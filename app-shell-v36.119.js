/* Hybrid Training v36.119 — modular product shell.
...
if(!core){console.error('Hybrid Training v36.119 shell: core bridge unavailable');return}
const BUILD='36.119',FOCUS_KEY='hybridTrainingWorkoutFocusV1';
...
function toggleCtxFlag(key){const c=explicitCtx(core.localISO(new Date())),active=key==='traveling'?(c.traveling===true||c.environment==='travel_indoor'):!!c[key];setCtx({[key]:!active})}
...
function syncPerformancePanel(){
 ...
 $('#v36101DataChips').innerHTML='<span class="v36101-data-chip '+(dc.rec?'good':'warn')+'">Recovery '+(dc.rec?'current':'missing')+'</span><span class="v36101-data-chip '+(dc.rows>=8?'good':'')+'">'+dc.rows+' recent log rows</span><span class="v36101-data-chip '+(dc.cond>=4?'good':'')+'">'+dc.cond+' conditioning rows</span><span class="v36101-data-chip">'+dc.wear+' wearable sessions</span>';
 const contextSummary=window.HybridDecisionIntegrity?.dayContextSummary?.(c),saved=$('#v36101ContextSaved');if(saved)saved.textContent=contextSummary?.meaningful?'Saved for today • '+contextSummary.text:'No context overrides saved today.';
 $$('[data-v36101-context]',p).forEach(b=>{const key=b.dataset.v36101Context,active=key==='traveling'?(c.traveling===true||c.environment==='travel_indoor'):!!c[key];b.classList.toggle('active',active)});$$('[data-v36101-time]',p).forEach(b=>b.classList.toggle('active',Number(c.timeBudget)===Number(b.dataset.v36101Time)));
}
...
function installSecondWorkoutPanel(){
 if(window.__v36123UseSecondaryMatrix){$('#v36105SecondWorkoutPanel')?.remove();return}
 if($('#v36105SecondWorkoutPanel'))return;const weak=$('#v36105WeakLinkPanel'),perf=$('#v36101PerformancePanel');const after=weak||perf;if(!after)return;const p=document.createElement('details');p.id='v36105SecondWorkoutPanel';p.className='v36105-second-panel hidden';p.innerHTML='<summary><div><span class="v36105-kicker">Another full session today</span><strong id="v36105SecondSummary">Second-workout guidance</strong></div><span class="v36105-second-state" id="v36105SecondState">—</span></summary><div class="v36105-second-body"><div class="v36105-second-head"><div><div class="v36105-kicker">Best full second workout</div><div class="v36105-main" id="v36105SecondName">—</div><div class="v36105-copy" id="v36105SecondReason"></div></div><div class="v36105-second-actions"><button class="btn primary" id="v36105SecondOpen" type="button">Open full workout</button><button class="btn" id="v36113ShortSecondaryJump" type="button">Short secondary</button></div></div><div class="v36105-second-metrics" id="v36105SecondMetrics"></div><div class="v36105-second-options" id="v36105SecondOptions"></div><div class="v36105-footnote">This opens a separate full workout. Short secondary is the smaller add-on. You can still choose a full second workout yourself when the app recommends the shorter option.</div></div>';after.insertAdjacentElement('afterend',p);p.addEventListener('click',e=>{const b=e.target.closest('[data-second-session]');if(!b)return;e.preventDefault();core.openFullSecondWorkout(b.dataset.secondSession,core.localISO(new Date()),b.dataset.secondOverride==='1');});$('#v36105SecondOpen')?.addEventListener('click',()=>{const b=$('#v36105SecondOpen'),s=b?.dataset?.session;if(s)core.openFullSecondWorkout(s,core.localISO(new Date()),b.dataset.secondOverride==='1')});$('#v36113ShortSecondaryJump')?.addEventListener('click',()=>{const el=$('.v3677-secondary');if(el){el.scrollIntoView({behavior:'smooth',block:'center'});const d=el.closest('details');if(d)d.open=true}});syncSecondWorkoutPanel();
}
function syncSecondWorkoutPanel(){
 if(window.__v36123UseSecondaryMatrix){$('#v36105SecondWorkoutPanel')?.remove();return}
 const p=$('#v36105SecondWorkoutPanel');if(!p)return;let r;try{r=core.fullSecondWorkoutRecommendation(core.localISO(new Date()))}catch(_){r=null}if(!r?.available){p.classList.add('hidden');return}p.classList.remove('hidden');const top=r.top,open=$('#v36105SecondOpen'),jump=$('#v36113ShortSecondaryJump'),alreadyTwo=Number(r.completedSessions)>=2,canOverride=!!top&&!r.recommended&&!alreadyTwo;if(!top){$('#v36105SecondSummary').textContent='No suitable full session';$('#v36105SecondState').textContent='Hold';$('#v36105SecondName').textContent='No recommendation';$('#v36105SecondReason').textContent=r.reason||'';if(open)open.classList.add('hidden');return}$('#v36105SecondSummary').textContent=r.recommended?top.session+' • '+top.label:'Full second workout not advised';const stateBadge=$('#v36105SecondState');if(stateBadge){stateBadge.textContent=r.recommended?'Feasible':alreadyTwo?'Two full sessions':r.blocked?'Not advised':'Low value';stateBadge.className='v36105-second-state '+(r.recommended?'good':alreadyTwo?'stop':'caution');}$('#v36105SecondName').textContent=top.session+' • '+top.label;$('#v36105SecondReason').textContent=(r.reason||'')+(canOverride?' You can still open it as a deliberate user override; the model warning remains in force.':'');if(open){open.dataset.session=top.session;open.dataset.secondOverride=canOverride?'1':'0';open.textContent=r.recommended?'Open full workout':canOverride?'Open full workout anyway':'Open full workout';open.className='btn'+(r.recommended?' primary':'');open.classList.toggle('hidden',alreadyTwo)}if(jump)jump.classList.toggle('hidden',alreadyTwo);const gap=Number(top.gapHours)||0,wait=Number(top.waitHours)||0,anchor=r.anchor?.days<9999?(r.anchor.days===1?'Tomorrow: '+r.anchor.session:r.anchor.days+'d: '+r.anchor.session):'No fixed anchor within 2 days';$('#v36105SecondMetrics').innerHTML='<span>Overlap '+Math.round((Number(top.overlap)||0)*100)+'%</span><span>Suggested separation ~'+gap+' h</span>'+(top.earliestStartClock?'<span>Earliest target '+esc(top.earliestStartClock)+'</span>':'')+(wait>.1?'<span>Wait ~'+wait+' h more</span>':'<span>Separation window okay</span>')+'<span>'+esc(anchor)+'</span>';if(top.dayPlanNote)$('#v36105SecondReason').textContent=(r.reason||'')+' '+top.dayPlanNote+(canOverride?' You can still open it as a deliberate user override; the model warning remains in force.':'');$('#v36105SecondOptions').innerHTML=(r.options||[]).slice(0,4).map((x,i)=>'<button type="button" class="btn '+(i===0&&r.recommended?'primary':'')+'" data-second-session="'+esc(x.session)+'" data-second-override="'+(!r.recommended?'1':'0')+'"'+(alreadyTwo?' disabled title="Two full workouts are already logged today."':(!r.recommended?' title="User override: the model does not advise another full session today."':''))+'><strong>'+esc(x.session)+'</strong><span>'+esc(x.label)+' • score '+esc(x.score)+(r.recommended?'':' • override available')+'</span></button>').join('');
}


// ---------- v36.119 diagnostics: plateau / gaps / asymmetry / cardio / data quality ----------
function installTrainingDiagnostics(){
 if($('#v36107TrainingDiagnostics'))return;const second=$('#v36105SecondWorkoutPanel'),weak=$('#v36105WeakLinkPanel'),perf=$('#v36101PerformancePanel'),after=second||weak||perf;if(!after)return;
 const p=document.createElement('details');p.id='v36107TrainingDiagnostics';p.className='v36107-diagnostics';p.innerHTML='<summary><div><span class="v36107-kicker">Training diagnostics</span><strong id="v36107DiagSummary">No major issue detected</strong></div><span class="v36107-diag-state" id="v36107DiagState">Current</span></summary><div class="v36107-diag-body"><div class="v36107-diag-grid" id="v36107DiagGrid"></div><div class="v36107-footnote">These are confidence-gated training signals, not diagnoses or rigid quotas. Old evidence gradually loses authority.</div></div>';after.insertAdjacentElement('afterend',p);syncTrainingDiagnostics();
}
function syncTrainingDiagnostics(){const p=$('#v36107TrainingDiagnostics');if(!p)return;const d=core.localISO(new Date());let plateau=[],gaps=[],asym=[],cardio=[],warnings=[];try{plateau=core.plateauDiagnostics(d)||[];gaps=core.missingStimulusDiagnostics(d)||[];asym=core.asymmetryDiagnostics(d)||[];cardio=core.cardioDoseGuidance(d)||[];warnings=core.dataQualityWarnings(d)||[]}catch(_){ }
 const topP=plateau[0],topG=gaps[0],topA=asym[0],topC=cardio[0],headline=topP?topP.exercise+': '+topP.label:topG?'Gap: '+topG.label:topA?'Asymmetry watch: '+topA.exercise:'No major issue detected';$('#v36107DiagSummary').textContent=headline;const stateEl=$('#v36107DiagState');stateEl.textContent=warnings.some(x=>x.level==='caution')?'Review':'Current';stateEl.className='v36107-diag-state '+(warnings.some(x=>x.level==='caution')?'caution':'good');
 const cards=[];if(topP)cards.push(['Plateau / progression',topP.exercise,topP.label+' — '+topP.action]);if(topG)cards.push(['Missing stimulus',topG.label,topG.note]);if(topA)cards.push(['Left / right watch',topA.exercise,topA.note+' Current repeated difference: ~'+topA.pct+'%.']);if(topC){const fam=String(topC.lane||'').toLowerCase()==='speed'?'Sprints / HIC adaptation':'LSS / Aerobic adaptation';cards.push([fam,prettyQuality(topC.lane),topC.note]);}if(warnings.length)cards.push(['Data quality',warnings[0].label,warnings[0].note]);$('#v36107DiagGrid').innerHTML=cards.length?cards.map(c=>'<div class="v36107-diag-card"><span>'+esc(c[0])+'</span><strong>'+esc(c[1])+'</strong><p>'+esc(c[2])+'</p></div>').join(''):'<div class="v36107-diag-card"><span>Current read</span><strong>Balanced / still learning</strong><p>No confidence-gated plateau, asymmetry, missing-stimulus, LSS/Sprints, or data-quality issue needs attention right now.</p></div>';
}
function prettyQuality(k){return String(k||'').replace(/([A-Z])/g,' $1').replace(/[_-]+/g,' ').replace(/^./,x=>x.toUpperCase())}

// ---------- Optional dynamic warm-up generated from the actual session ----------
function currentLogExerciseNames(){return $$('#plannedExercises>.log-exercise').map(c=>core.currentCardExercise(c)).filter(Boolean)}
function installWarmupPanel(){if($('#v36107Warmup'))return;const plan=$('#plannedExercises');if(!plan)return;const d=document.createElement('details');d.id='v36107Warmup';d.className='v36107-warmup';d.innerHTML='<summary><div><span>Optional dynamic warm-up</span><strong id="v36107WarmupTitle">Session-specific prep</strong></div><span id="v36107WarmupMinutes">—</span></summary><div class="v36107-warmup-body"><ol id="v36107WarmupSteps"></ol><p id="v36107WarmupNote"></p></div>';plan.insertAdjacentElement('beforebegin',d);syncWarmupPanel()}
function syncWarmupPanel(){const p=$('#v36107Warmup');if(!p)return;const date=$('#logDate')?.value||core.localISO(new Date()),session=$('#logSession')?.value||'',names=currentLogExerciseNames();let w;try{w=core.dynamicWarmupForDate(date,session,names)}catch(_){w=null}if(!w)return;$('#v36107WarmupTitle').textContent=(w.firstExercise?('Prep for '+w.firstExercise):w.session);$('#v36107WarmupMinutes').textContent=w.minutes;$('#v36107WarmupSteps').innerHTML=(w.steps||[]).map(x=>'<li>'+esc(x)+'</li>').join('');$('#v36107WarmupNote').textContent=w.note||''}

// ---------- Exercise-level longitudinal History ----------
function historyExerciseNames(){const logs=state().logs||[];return [...new Set(logs.filter(r=>r.exercise&&!core.rowIsAdaptiveSecondary(r)).map(r=>r.exercise))].sort((a,b)=>a.localeCompare(b))}
function installHistoryLongitudinal(){if($('#v36107HistoryLong'))return;const view=$('#history');if(!view)return;const first=view.querySelector('.panel');if(!first)return;const d=document.createElement('details');d.id='v36107HistoryLong';d.className='v36107-history-long';d.innerHTML='<summary><div><span>Exercise history</span><strong id="v36107HistoryLongSummary">Progress and recent context</strong></div></summary><div class="v36107-history-long-body"><label>Exercise<select id="v36107HistoryExercise"></select></label><div id="v36107HistorySnapshot"></div></div>';first.insertAdjacentElement('afterend',d);const sel=$('#v36107HistoryExercise');sel.addEventListener('change',syncHistoryLongitudinal);syncHistoryLongitudinal()}
function syncHistoryLongitudinal(){const sel=$('#v36107HistoryExercise'),host=$('#v36107HistorySnapshot');if(!sel||!host)return;const names=historyExerciseNames(),prior=sel.value;if(sel.options.length!==names.length||names.some((n,i)=>sel.options[i]?.value!==n)){sel.innerHTML=names.map(n=>'<option value="'+esc(n)+'">'+esc(n)+'</option>').join('');if(names.includes(prior))sel.value=prior}const name=sel.value||names.at(-1);if(!name){host.innerHTML='<div class="muted">No exercise history yet.</div>';return}let x;try{x=core.exerciseLongitudinalSnapshot(name,core.localISO(new Date()))}catch(_){x=null}if(!x)return;const chips=[''+x.exposures+' workouts',x.daysSince>=9999?'No recent workout':x.daysSince+'d since last','Trend '+(x.direction==='up'?'↑':x.direction==='down'?'↓':'→')+' '+x.trendPct+'%',x.avgRecentEffort?'Avg effort '+x.avgRecentEffort:''].filter(Boolean);let notes=[];if(x.plateau)notes.push('<strong>Plateau:</strong> '+esc(x.plateau.label)+' — '+esc(x.plateau.action));if(x.weakLink)notes.push('<strong>Possible weak area:</strong> '+esc(x.weakLink.muscleLabel));if(x.asymmetry)notes.push('<strong>Side-to-side difference:</strong> '+esc(x.asymmetry.side)+' side ~'+esc(x.asymmetry.pct)+'%');if(x.painFlagsRecent)notes.push('<strong>Pain notes:</strong> '+x.painFlagsRecent+' recent flag'+(x.painFlagsRecent===1?'':'s'));host.innerHTML='<div class="v36107-history-chips">'+chips.map(c=>'<span>'+esc(c)+'</span>').join('')+'</div><div class="v36107-history-notes">'+(notes.length?notes.join('<br>'):'No clear plateau, weak area, side-to-side difference, or recent pain note for this exercise.')+'</div>'}
function addHistoryStimulusSummaries(){let groups=[];try{groups=core.historySessionGroups()||[]}catch(_){return}$$('#historyBody .history-session-header').forEach(h=>{if($('.v36107-stimulus',h))return;const sid=$('.edit-session',h)?.dataset.sessionId,g=groups.find(x=>x.sessionId===sid);if(!g)return;const s=core.workoutStimulusSummary(g);if(!s)return;const d=document.createElement('div');d.className='v36107-stimulus';d.innerHTML='<strong>Stimulus:</strong> '+esc(s.note);$('.history-session-meta',h)?.appendChild(d)});const mobile=$('#historyMobileCards');if(mobile)$$('.history-mobile-edit',mobile).forEach(e=>{const row=e.closest('.mobile-data-row');if(!row||$('.v36107-stimulus',row))return;const g=groups.find(x=>x.sessionId===e.dataset.sessionId),s=g&&core.workoutStimulusSummary(g);if(!s)return;const d=document.createElement('div');d.className='v36107-stimulus';d.innerHTML='<strong>Stimulus:</strong> '+esc(s.note);e.closest('.inline-actions')?.insertAdjacentElement('beforebegin',d)})}
...
function installWorkoutBar(){
 ...
 const b=document.createElement('div');b.id='v36101WorkoutModeBar';b.className='v36101-workout-mode-bar';b.innerHTML='<div class="v36101-workout-mode-main"><div class="v36101-workout-mode-name"><span>Workout mode</span><strong id="v36101WorkoutName">Workout</strong><div class="v36101-mode-progress" id="v36101WorkoutProgress"></div><div class="v36101-rest-clock" id="v36101RestClock"></div></div><div class="v36101-workout-mode-nav"><button class="btn" id="v36101PrevExercise" type="button" aria-label="Previous exercise">‹</button><button class="btn" id="v36101NextExercise" type="button" aria-label="Next exercise">›</button><button class="btn v36129-add-exercise" id="v36129AddExercise" type="button">+ Exercise</button><button class="btn primary" id="v36101FinishJump" type="button">Finish</button></div></div>';
 tracker.insertAdjacentElement('beforebegin',b);$('#v36101PrevExercise').addEventListener('click',()=>setFocus(focusIndex-1,true));$('#v36101NextExercise').addEventListener('click',()=>setFocus(focusIndex+1,true));$('#v36129AddExercise').addEventListener('click',()=>{const before=exerciseCards().length,trigger=$('#addLogExercise');if(!trigger)return;trigger.click();setTimeout(()=>{const cards=exerciseCards();if(cards.length<=before)return;decorateAllCards();setFocus(cards.length-1,true);$('.le-ex',cards.at(-1))?.focus()},0)});$('#v36101FinishJump').addEventListener('click',()=>{const d=$('#v36101FinishDetails');if(d){d.open=true;d.scrollIntoView({behavior:'smooth',block:'start'})}});
...
function enhanceHistoryComparisons(){
 ...
 addHistoryStimulusSummaries();installHistoryLongitudinal();syncHistoryLongitudinal();
}

// ---------- Wiring ----------
function boot(){
 installUpdateBanner();installPerformancePanel();installWeakLinkPanel();installSecondWorkoutPanel();installTrainingDiagnostics();installWarmupPanel();installWorkoutBar();installFinishDisclosure();installUndoCoverage();decorateAllCards();syncWorkoutMode();syncPerformancePanel();syncWeakLinkPanel();syncSecondWorkoutPanel();syncTrainingDiagnostics();syncWarmupPanel();enhanceHistoryComparisons();installHistoryLongitudinal();
 const plan=$('#plannedExercises');if(plan)new MutationObserver(()=>{clearTimeout(window.__v36101CardTimer);window.__v36101CardTimer=setTimeout(()=>{decorateAllCards();syncWarmupPanel()},0)}).observe(plan,{childList:true,subtree:true});
 ...
 ['startWorkoutTracker','pauseWorkoutTracker','resumeWorkoutTracker','discardWorkoutTracker'].forEach(id=>$('#'+id)?.addEventListener('click',()=>setTimeout(syncWorkoutMode,30)));$('#saveSession')?.addEventListener('click',()=>setTimeout(()=>{syncWorkoutMode();syncWeakLinkPanel();syncSecondWorkoutPanel();syncTrainingDiagnostics();syncHistoryLongitudinal()},450));
 document.addEventListener('click',e=>{if(e.target.closest('[data-view="log"],#quickLog,#activeWorkoutReturn'))setTimeout(()=>{syncWorkoutMode();decorateAllCards();syncWarmupPanel()},60);if(e.target.closest('[data-view="history"],[data-mobile-view="history"]'))setTimeout(enhanceHistoryComparisons,80)});
