/* Hybrid Training v36.101 — modular product shell.
...
if(!core){console.error('Hybrid Training v36.101 shell: core bridge unavailable');return}
const BUILD='36.101',FOCUS_KEY='hybridTrainingWorkoutFocusV1';
...
function setCtx(patch){const d=core.localISO(new Date());core.setDayContextPatch(d,patch);syncPerformancePanel()}
...
// ---------- Wiring ----------
function boot(){
 installUpdateBanner();installPerformancePanel();installWorkoutBar();installFinishDisclosure();installUndoCoverage();decorateAllCards();syncWorkoutMode();syncPerformancePanel();enhanceHistoryComparisons();
 ...
 ['startWorkoutTracker','pauseWorkoutTracker','resumeWorkoutTracker','discardWorkoutTracker','saveSession'].forEach(id=>$('#'+id)?.addEventListener('click',()=>setTimeout(syncWorkoutMode,30)));
