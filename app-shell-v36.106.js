/* Hybrid Training v36.106 — modular product shell.
...
if(!core){console.error('Hybrid Training v36.106 shell: core bridge unavailable');return}
const BUILD='36.106',FOCUS_KEY='hybridTrainingWorkoutFocusV1';
...
// ---------- v36.106 diagnostics: plateau / gaps / asymmetry / cardio / data quality ----------
function installTrainingDiagnostics(){
 if($('#v36106TrainingDiagnostics'))return;const second=$('#v36105SecondWorkoutPanel'),weak=$('#v36105WeakLinkPanel'),perf=$('#v36101PerformancePanel'),after=second||weak||perf;if(!after)return;
 const p=document.createElement('details');p.id='v36106TrainingDiagnostics';p.className='v36106-diagnostics';p.innerHTML='<summary><div><span class="v36106-kicker">Training diagnostics</span><strong id="v36106DiagSummary">No major issue detected</strong></div><span class="v36106-diag-state" id="v36106DiagState">Current</span></summary><div class="v36106-diag-body"><div class="v36106-diag-grid" id="v36106DiagGrid"></div><div class="v36106-footnote">These are confidence-gated training signals, not diagnoses or rigid quotas. Old evidence gradually loses authority.</div></div>';after.insertAdjacentElement('afterend',p);syncTrainingDiagnostics();
}
function syncTrainingDiagnostics(){const p=$('#v36106TrainingDiagnostics');if(!p)return;const d=core.localISO(new Date());let plateau=[],gaps=[],asym=[],cardio=[],warnings=[];try{plateau=core.plateauDiagnostics(d)||[];gaps=core.missingStimulusDiagnostics(d)||[];asym=core.asymmetryDiagnostics(d)||[];cardio=core.cardioDoseGuidance(d)||[];warnings=core.dataQualityWarnings(d)||[]}catch(_){ }
 const topP=plateau[0],topG=gaps[0],topA=asym[0],topC=cardio[0],headline=topP?topP.exercise+': '+topP.label:topG?'Gap: '+topG.label:topA?'Asymmetry watch: '+topA.exercise:'No major issue detected';$('#v36106DiagSummary').textContent=headline;const stateEl=$('#v36106DiagState');stateEl.textContent=warnings.some(x=>x.level==='caution')?'Review':'Current';stateEl.className='v36106-diag-state '+(warnings.some(x=>x.level==='caution')?'caution':'good');
 const cards=[];if(topP)cards.push(['Plateau / progression',topP.exercise,topP.label+' — '+topP.action]);if(topG)cards.push(['Missing stimulus',topG.label,topG.note]);if(topA)cards.push(['Left / right watch',topA.exercise,topA.note+' Current repeated difference: ~'+topA.pct+'%.']);if(topC)cards.push(['Cardio adaptation',prettyQuality(topC.lane),topC.note]);if(warnings.length)cards.push(['Data quality',warnings[0].label,warnings[0].note]);$('#v36106DiagGrid').innerHTML=cards.length?cards.map(c=>'<div class="v36106-diag-card"><span>'+esc(c[0])+'</span><strong>'+esc(c[1])+'</strong><p>'+esc(c[2])+'</p></div>').join(''):'<div class="v36106-diag-card"><span>Current read</span><strong>Balanced / still learning</strong><p>No confidence-gated plateau, asymmetry, missing-stimulus, cardio, or data-quality issue needs attention right now.</p></div>';
}
function prettyQuality(k){return String(k||'').replace(/([A-Z])/g,' $1').replace(/[_-]+/g,' ').replace(/^./,x=>x.toUpperCase())}
...
function installWarmupPanel(){if($('#v36106Warmup'))return;const plan=$('#plannedExercises');if(!plan)return;const d=document.createElement('details');d.id='v36106Warmup';d.className='v36106-warmup';d.innerHTML='<summary><div><span>Optional dynamic warm-up</span><strong id="v36106WarmupTitle">Session-specific prep</strong></div><span id="v36106WarmupMinutes">—</span></summary><div class="v36106-warmup-body"><ol id="v36106WarmupSteps"></ol><p id="v36106WarmupNote"></p></div>';plan.insertAdjacentElement('beforebegin',d);syncWarmupPanel()}
function syncWarmupPanel(){const p=$('#v36106Warmup');if(!p)return;const date=$('#logDate')?.value||core.localISO(new Date()),session=$('#logSession')?.value||'',names=currentLogExerciseNames();let w;try{w=core.dynamicWarmupForDate(date,session,names)}catch(_){w=null}if(!w)return;$('#v36106WarmupTitle').textContent=(w.firstExercise?('Prep for '+w.firstExercise):w.session);$('#v36106WarmupMinutes').textContent=w.minutes;$('#v36106WarmupSteps').innerHTML=(w.steps||[]).map(x=>'<li>'+esc(x)+'</li>').join('');$('#v36106WarmupNote').textContent=w.note||''}
...
function installHistoryLongitudinal(){if($('#v36106HistoryLong'))return;const view=$('#history');if(!view)return;const first=view.querySelector('.panel');if(!first)return;const d=document.createElement('details');d.id='v36106HistoryLong';d.className='v36106-history-long';d.innerHTML='<summary><div><span>Exercise longitudinal view</span><strong id="v36106HistoryLongSummary">Progress, exposure and context</strong></div></summary><div class="v36106-history-long-body"><label>Exercise<select id="v36106HistoryExercise"></select></label><div id="v36106HistorySnapshot"></div></div>';first.insertAdjacentElement('afterend',d);const sel=$('#v36106HistoryExercise');sel.addEventListener('change',syncHistoryLongitudinal);syncHistoryLongitudinal()}
function syncHistoryLongitudinal(){const sel=$('#v36106HistoryExercise'),host=$('#v36106HistorySnapshot');if(!sel||!host)return;const names=historyExerciseNames(),prior=sel.value;if(sel.options.length!==names.length||names.some((n,i)=>sel.options[i]?.value!==n)){sel.innerHTML=names.map(n=>'<option value="'+esc(n)+'">'+esc(n)+'</option>').join('');if(names.includes(prior))sel.value=prior}const name=sel.value||names.at(-1);if(!name){host.innerHTML='<div class="muted">No exercise history yet.</div>';return}let x;try{x=core.exerciseLongitudinalSnapshot(name,core.localISO(new Date()))}catch(_){x=null}if(!x)return;const chips=[''+x.exposures+' exposures',x.daysSince>=9999?'No recent exposure':x.daysSince+'d since last','Trend '+(x.direction==='up'?'↑':x.direction==='down'?'↓':'→')+' '+x.trendPct+'%',x.avgRecentEffort?'Avg effort '+x.avgRecentEffort:''].filter(Boolean);let notes=[];if(x.plateau)notes.push('<strong>Plateau:</strong> '+esc(x.plateau.label)+' — '+esc(x.plateau.action));if(x.weakLink)notes.push('<strong>Weak-link watch:</strong> '+esc(x.weakLink.muscleLabel));if(x.asymmetry)notes.push('<strong>Asymmetry watch:</strong> '+esc(x.asymmetry.side)+' side ~'+esc(x.asymmetry.pct)+'%');if(x.painFlagsRecent)notes.push('<strong>Pain context:</strong> '+x.painFlagsRecent+' recent flag'+(x.painFlagsRecent===1?'':'s'));host.innerHTML='<div class="v36106-history-chips">'+chips.map(c=>'<span>'+esc(c)+'</span>').join('')+'</div><div class="v36106-history-notes">'+(notes.length?notes.join('<br>'):'No confidence-gated plateau, weak-link, asymmetry or pain-context flag for this exercise.')+'</div>'}
function addHistoryStimulusSummaries(){let groups=[];try{groups=core.historySessionGroups()||[]}catch(_){return}$$('#historyBody .history-session-header').forEach(h=>{if($('.v36106-stimulus',h))return;const sid=$('.edit-session',h)?.dataset.sessionId,g=groups.find(x=>x.sessionId===sid);if(!g)return;const s=core.workoutStimulusSummary(g);if(!s)return;const d=document.createElement('div');d.className='v36106-stimulus';d.innerHTML='<strong>Stimulus:</strong> '+esc(s.note);$('.history-session-meta',h)?.appendChild(d)});const mobile=$('#historyMobileCards');if(mobile)$$('.history-mobile-edit',mobile).forEach(e=>{const row=e.closest('.mobile-data-row');if(!row||$('.v36106-stimulus',row))return;const g=groups.find(x=>x.sessionId===e.dataset.sessionId),s=g&&core.workoutStimulusSummary(g);if(!s)return;const d=document.createElement('div');d.className='v36106-stimulus';d.innerHTML='<strong>Stimulus:</strong> '+esc(s.note);e.closest('.inline-actions')?.insertAdjacentElement('beforebegin',d)})}
